...
// ==========================================
// 🔥 FIX: S3 OVERNIGHT GUARD
// ==========================================

function isS3ActiveNow() {
  const now = new Date();
  const mins = now.getHours() * 60 + now.getMinutes();
  return (mins >= 1290 || mins < 480);
}
...
setInterval(() => {
  ...
  if (newKey !== currentDateKey) {
    ...
    // ⭐ CRITICAL FIX: block midnight flip during S3
    if (isS3ActiveNow()) {
      return;
    }
